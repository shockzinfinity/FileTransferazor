...
| Database | PostgreSQL | Npgsql 10.x |
| File Storage | AWS S3 / 로컬 파일시스템 | 설정으로 전환 |
...
│   ├── Services/
│   │   ├── IFileStorageProvider.cs   # 파일 저장소 추상화 인터페이스
│   │   ├── AwsS3FileManager.cs      # IFileStorageProvider 구현 (AWS S3)
│   │   ├── LocalFileStorageProvider.cs # IFileStorageProvider 구현 (로컬 파일시스템)
...
- PostgreSQL (Docker 권장, 또는 외부 인스턴스)
- AWS 계정 (S3 사용 시, Systems Manager 권한) — Local 모드에서는 불필요

### 1. 데이터베이스 설정

```bash
# PostgreSQL Docker 컨테이너 실행
docker run -e "POSTGRES_USER=admin" -e "POSTGRES_PASSWORD=yourpassword" \
  -e "POSTGRES_DB=fileTransferazorDb" \
  -p 5432:5432 --name filetransferazor-db \
  -d postgres:17
...
{
  "ConnectionStrings": {
    "DefaultConnection": "Host=localhost;Port=5432;Database=fileTransferazorDb;Username=admin;Password=yourpassword;Pooling=true;Minimum Pool Size=2;Maximum Pool Size=10;Keepalive=30;Timeout=15;Command Timeout=30"
  },
  "FileStorage": {
    "Provider": "Local"
  },
  "LocalFileStorage": {
    "StoragePath": "/tmp/filetransferazor-storage"
  },
  "AwsS3": {
...
#### FileStorage.Provider 설정

| 값 | 저장소 | 필요 설정 | 용도 |
|----|--------|----------|------|
| `"Local"` | 로컬 파일시스템 | `LocalFileStorage.StoragePath` | 개발/테스트, NAS |
| `"S3"` | AWS S3 | `AwsS3.BucketName` + AWS credentials | 프로덕션 |

S3로 전환 시 `"Provider": "Local"` → `"Provider": "S3"`로 변경하고, AWS credentials를 환경변수 또는 `~/.aws/credentials`로 설정하면 됩니다.
...
| 클래스 | 설정 섹션 | 용도 |
|--------|----------|------|
| `AwsS3Options` | `AwsS3` | S3 버킷 이름 (Provider=S3일 때) |
| `LocalFileStorageOptions` | `LocalFileStorage` | 로컬 저장 경로 (Provider=Local일 때) |
